...
| Sim throughput, Node (CI gate) | ≥ 2,000 ticks/s on a GitHub runner, 64×40 world, 200 organisms                    | **1,510–1,680 ticks/s** (raw Node, no test framework), below budget on this machine. `npx vitest run test/invariants/throughput.test.js`'s own number is far lower still (~200–300 ticks/s) — a separate, pre-existing, already root-caused vitest tax (see "Findings" below), not a second data point on core throughput                                                                                                                                                                                                                                         | `node scripts/perf.mjs`                                              |
...
**Not a code regression — a measurement environment finding, already
root-caused (P1-10's log):** `npx vitest run test/invariants/throughput.test.js`
measured 203–313 ticks/s across four runs during this task (this
machine, this session), vs. raw Node's 1,510–1,680 ticks/s on the
identical scenario. That ~5–8× gap is the same vitest module-transform
tax P1-10 root-caused with a minimal repro (a ~14.8× slowdown for
cross-module calls under vitest's SSR/vite-node pipeline, independent of
CPU load or warm-up) — present in every task's log from P3-10 onward as
"this machine's ongoing CPU contention." `npm test` this task: **451
passed, 1 failed** — only the pre-existing throughput invariant, for the
reasons above, not a regression introduced by this diff (confirmed by
`git stash`: pre-fix `pheromone.js` reproduces the same behaviour). SPEC
§8's own gate targets "a GitHub runner," not this shared desktop; the
committed assertion in `throughput.test.js` is left at exactly `2000`
(unchanged) since that runner, not this machine, is the real arbiter.
