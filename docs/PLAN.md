...
- Phase 6 (added 2026-09-20, after the Phase 5 build was reviewed in the browser) → plant stock is kept as the same `Float32` per-tile energy stock (SPEC §4.4), only its scale changes: `terrain.plantCap` becomes tens of energy units per tile instead of ≤ 1, so the ledger identity and every formula are unchanged and no "energy per plant unit" multiplier is introduced. Lifespan stays a phenotype in days but its range moves to years (`[36, 96]` days at 24 days/year); maturity stays a fraction of lifespan. Rationale: the shipped world was an immigration treadmill (see the Phase 6 preamble); fixing the scale of the existing stocks is a config change, a multiplier would be a second place the same fact lives.
...
| `world.counters.deaths`, `world.counters.deathAgePct` | 0, 0 | count, Σ round(100·age/lifespan) | no (counters, not config) | P6-01 | — |
| `terrain.edgeWetness` | 0.05 | noise value added × sin(πx/w) | yes | P6-05 | P6-05 |
| Phase 6 retuned ⚠️ defaults (`terrain.plantCap`, `plants.growth`, `organisms.biteSize`, `metabolism.base`, `phenotype.lifespan`, `phenotype.maturity`, `regrowth.zeroThreshold`, `interventions.meadow.plants`, `breeding.*`, `predation.*`, `immigration.*`, `genesis.*`) | see P6-03/P6-04/P6-05 | as before | yes | earlier | P6-03..P6-05 |
...
## Phase 6 — Ecology rebalance

**Why this phase exists.** Fable's review of the shipped world on 2026-09-20 (seed 1, default config, 4 in-world years, `scripts/headless.mjs`-style probe) found that the ecology is not self-sustaining: 152 of 174 founders starve by day 6; from then on the population sits at the immigration floor (20 herbivores + 4 carnivores), plants sit at 99–100% of cap on every tile, the mean age of anything alive is ~1.5 days, and over 4 years there are 116 births against 107 immigrations. `docs/sweeps/p5-05-before.txt` shows the same on all 40 seeds at 100,000 ticks (pop 20–37, plants 99.9%, born ~50, starved ~450, immig ~65). The Phase 3 soak invariants pass only because immigration keeps the world alive; none of them asserts that births outnumber immigrations or that plants are ever grazed down. The user-visible symptoms are: plants regrow to 100% every day so nothing migrates; old-age deaths by day 14 (lifespan is 3–9 *days*, a year is 24 days); everyone huddles at the map edges (immigrants arrive at edges and die within two days); and 22 grazers / 3 hunters after four years (the immigration floors, not an equilibrium).

Root causes are scale, not bugs: a full grass tile holds 1 energy unit while an organism holds up to 150 and bites 0.1 per tick, so a grazer needs 150 tiles to fill up; P1-11 compensated by raising `plants.growth` to 0.6/tick (an empty tile refills in ~2 ticks) and shortening lifespans so founders bred before dying. Each knob was tuned against the others' wrong scale. Fable's probes (recorded in the P6-03 task text) show that rescaling plant stock to tens of energy units per tile, slowing regrowth to days, cheapening the metabolic floor, and moving lifespan to years makes births outnumber immigrations 40:1 within two in-world years, drops plant fill to 30–70%, and halves the edge share, at which point the world boom-busts (a real predator/prey/resource oscillation) instead of flat-lining. Phase 6 makes that rebalance, adds the soak invariants that would have caught the treadmill, then damps the oscillation, gives hunters a viable niche, and enlarges genesis.

**Rules for every task in this phase.** Determinism (SPEC §3.1, §6.3) applies to every `src/core` change: randomness only through `world.rng` in fixed order, slot-order loops, lowest-slot ties, no per-tick allocation, new state covered by `hash()` and `save.js`. Config is data (SPEC §3.7): tuning tasks change `src/core/config.js` defaults only, never a formula. Tuning changes default values, and `diffConfig` stores a save's config as a diff against `DEFAULTS`, so Phase 6 changes what a pre-Phase-6 seed replays into; P6-06 owns that (save `VERSION` bump). Sweeps are `node scripts/sweep.mjs --seeds 1..40 --ticks 100000` at the default size unless the task says otherwise; each tuning task saves the before and after tables to `docs/sweeps/p6-<nn>-before.txt` / `-after.txt` and summarises them in `docs/tuning.md` under a `## P6-<nn>` heading, as P3-10 and P5-05 did. Tuning tasks may take more than one iteration; each iteration's sweep goes in `docs/tuning.md`, only the final one in the commit message.

### P6-01: Ecology health metrics — death-age counters and the sweep columns that expose a treadmill
**Goal:** Make the treadmill measurable: add the counters and report/sweep columns that P6-03's invariants and every later tuning task will assert on.
**Files touched:** `src/core/world.js` (two new `COUNTER_KEYS` entries), `scripts/lib/report.mjs`, `scripts/sweep.mjs`, `scripts/headless.mjs`, `test/unit/world.test.js` (extend), `test/unit/report.test.js` (extend).
**Design constraints:**
- Determinism (SPEC §3.1): counters are plain integers updated in `resolve()` in slot order, at the point each death is settled; nothing else changes. Follow the existing `COUNTER_KEYS` pattern exactly (they are packed into saves in `Object.keys()` order by `save.js` §counters, so check that pattern and its `save.test.js` round-trip still passes; the counter section id and layout must not change shape beyond the two new keys).
- New counters: `deaths` (every death of any cause, incremented where the death is settled) and `deathAgePct` (per death, add `Math.round(100 * age / lifespanTicks)`, clamped to `[0, 200]`; integer so the sum cannot lose precision in a 4-byte pack). Mean age-at-death as a fraction of lifespan is `deathAgePct / deaths`, computed in the report, never in core.
- `scripts/lib/report.mjs` `ecologyReport()` gains: `bornPerImmig` = `born / max(1, immigrations)`; `deathAgeMeanPct` = `deaths > 0 ? deathAgePct / deaths : 0`; `edgePct` = percentage of living organisms whose `x` or `y` is within `opts.edgeMargin` tiles (default 20, a report option, not a core config key) of any map edge; `plantsAvgPct` = mean of `world.stats.plantsFraction` over the samples currently held in the stats ring (`world.stats.n` samples, so at most `stats.historyLength × stats.sampleEvery` = 30,720 ticks of history) × 100; `capacityRefused` already exists — keep it. `scripts/sweep.mjs` gains columns `bornPerImmig`, `deathAge%`, `edge%`, `plantsAvg%`, `minPopY2` (minimum `store.count` sampled every 600 ticks from tick 43,200 = start of year 2 onward, so a boom-bust crash shows up even if the run ends on a peak; `0` if the run is shorter than 43,200 ticks) and `refused` (`capacityRefused`), plus their means in the `---` summary lines, and the JSON rows carry the same keys. `scripts/headless.mjs` prints the new report fields on one extra line.
**Acceptance tests:** `test/unit/world.test.js` (extend): `"every death increments deaths and adds its age as a percentage of lifespan to deathAgePct"` (isolate: aging on, lifespan overridden short via `phenotype.lifespan`, one organism; then a second organism that starves via `metabolism` — assert `deaths === 2`, `deathAgePct` equals the sum of the two rounded percentages). `test/unit/report.test.js` (extend): `"bornPerImmig, deathAgeMeanPct, edgePct and plantsAvgPct are derived from the world"` (build a 64×40 world with `organisms` placed at known positions, set counters directly, assert the four numbers), `"minPopY2 is 0 for runs shorter than a year and the sampled minimum otherwise"` (unit-test the sampling helper, exported from `scripts/sweep.mjs`'s lib or from `scripts/lib/report.mjs`, with a fake sampler).
**Out of scope:** Any change to defaults, mechanics, the soak tests, or the UI.
**Verification:** `npm run typecheck && npm run lint && npm test`; `npm run headless -- --seed 1 --ticks 30000` prints the new line and `bornPerImmig` is below 2 (this is the treadmill, expected at this point; record the printed line in the log); `node scripts/sweep.mjs --seeds 1..3 --ticks 5000` prints the new columns.
**Depends on:** none

### P6-02: Plant stock in energy units — remove every cap ≤ 1 assumption
**Goal:** Make the plant stock scale-free so that P6-03 can raise `terrain.plantCap` to tens of energy units per tile without any consumer assuming a full tile is `1.0`.
**Files touched:** `src/render/terrain-layer.js`, `src/render/renderer.js` (only if that is where the cap table has to be plumbed), `src/sim/snapshot.js` (only if the cap table travels in the snapshot header), `src/ui/app.js` (tile tooltip), `src/core/interventions.js` (meadow, only if needed), `src/core/config.js` (DOCS `units` strings only, no default changes), `test/unit/terrain-layer.test.js` (extend), `test/unit/tooltip.test.js` (create) or `test/unit/smoke.test.js` (extend, whichever already exercises the tile tooltip), `test/unit/ecology.test.js` (extend), `test/unit/interventions.test.js` (extend).
**Design constraints:**
- Behaviour must be byte-identical at the current defaults (every cap ≤ 1): `npm run headless -- --ticks 5000` prints the same hash before and after this task. Record both hashes in the log.
- Consumers that currently treat the raw plant value as a fraction: `src/render/terrain-layer.js` (tints grass/scrub/mud by `p`, must tint by `p / plantCap[terrain]`), `src/ui/app.js` (passes the raw value to `tileTooltipText` as `plantsFraction`, must pass `p / plantCap[terrain]`). The cap table must reach the renderer and the app without importing `DEFAULTS` from core into `src/render`/`src/ui` (CLAUDE.md boundary: pure helpers only, never a `World`, and the running world's config may differ from `DEFAULTS`). Interpretation point — pick the smallest plumbing that works and record it: the `loaded` event already carries world dimensions from the worker to the app (`src/sim/protocol.js`); extending that payload with `plantCap` (six numbers) and handing it to the renderer's terrain layer is the expected route. Cap 0 terrains (water, sand, rock) never tint (guard the division).
- `terrain.plantCap`, `plants.growth`, `organisms.biteSize`, `regrowth.zeroThreshold`, `interventions.meadow.plants`, `famine.plantFraction` DOCS entries: state units as "energy units per tile" / "energy units per tick" / "energy units" where they are absolute, and "fraction of Σcap" where relative, so P6-03 tunes them knowingly. The meadow intervention raises tiles to `interventions.meadow.plants` (absolute) clamped to the grass cap; it is already correct for caps > 1 — confirm by test, change nothing unless the test fails.
- The energy ledger identity (CLAUDE.md, SPEC §4.4) must hold unchanged with caps > 1: `test/invariants/energy.test.js` stays green, and the new ecology test below runs it at cap 40.
**Acceptance tests:** `test/unit/terrain-layer.test.js` (extend): `"tint is by fraction of the tile's cap, so a full scrub tile at cap 24 is tinted as fully as a full grass tile at cap 40"` and `"a cap-0 terrain is never tinted by its plant value"`. Tile tooltip test: `"the tile tooltip shows plants as a percentage of the tile's cap"` (grass at cap 40 holding 10 shows `plants 25%`). `test/unit/ecology.test.js` (extend): `"with plantCap[GRASS] = 40 a grass tile grows toward 40, a bite removes biteSize energy units, and the ledger identity holds to 1e-6"` (config overrides only, `isolate(['plants','metabolism'])` or the nearest existing pattern, 2,000 ticks). `test/unit/interventions.test.js` (extend): `"meadow raises tiles to meadow.plants energy units clamped at the grass cap"` (cap 40, `meadow.plants` 20).
**Out of scope:** Changing any default. Touching `src/core/ecology.js` formulas.
**Verification:** `npm run typecheck && npm run lint && npm test && npm run build`; `npm run headless -- --ticks 5000` hash unchanged from before the task (record both); in the browser (`npm run dev`), hover a grass tile and confirm the tooltip still reads a sensible percentage.
**Depends on:** P6-01

### P6-03: Rebalance — ecology health invariants and the scale retune
**Goal:** Retune the ecological ⚠️ keys to the new scale so that births, not immigration, sustain the population, plants are a resource that runs out, and lifespans are measured in years; and add the soak invariants that fail on the old defaults.
**Files touched:** `src/core/config.js` (⚠️ defaults and their comments only), `test/soak/health.test.js` (new), `test/soak/ecology.test.js` and `test/soak/survival.test.js` (re-pin seeds with reasons if the old pinned seeds no longer pass), `docs/tuning.md`, `docs/sweeps/p6-03-before.txt`, `docs/sweeps/p6-03-after.txt`.
**Design constraints:**
- Tests first, in the P1-11 pattern: write `test/soak/health.test.js` and run it against the current defaults; it must fail on the treadmill assertions (record the failing numbers in the log), then tune until it passes.
- `test/soak/health.test.js` — for each pinned seed, run 100,000 ticks at the default size (reuse `test/soak/ecology.test.js`'s loop shape and 1,200,000 ms timeout; sample `store.count` every 600 ticks from tick 43,200 for `minPopY2`), then assert: `"births outnumber immigrations at least ten to one"` (`counters.born ≥ 10 × counters.immigrations`); `"plants are grazed: mean plant fill over the last 30k ticks is between 15% and 90% of cap"` (from `world.stats.plantsFraction` over `world.stats.n` samples); `"immigration is a backstop, not a supply: at most 30 immigration events"`; `"the population is not pinned to the edges: at most 60% of living organisms are within 20 tiles of an edge at the end"`; `"herbivores and carnivores are both alive at the end"`; `"the energy ledger closes to 1e-3 relative"`. Two pinned seeds, chosen from the after-sweep and recorded with reasons as `ecology.test.js` does.
- Starting point (Fable's 48-day probes on seeds 1 and 8, 2026-09-20; these are a starting point, not the answer): `terrain.plantCap` `[0, 0, 14, 40, 24, 0]` (a full grass tile = 40 energy units, ~¼ of an organism's `energyMaxBase`); `plants.growth` 0.015 energy units/tick at L=1 (an empty grass tile needs ~2,700 lit ticks ≈ 3 in-world days to refill, so grazed ground stays grazed for days); `organisms.biteSize` 0.3; `metabolism.base` 0.008; `phenotype.lifespan` `[36, 96]` days (1.5–4 years at 24 days/year); `phenotype.maturity` `[0.04, 0.15]` of lifespan (1.4–14 days, so founders breed within their first two weeks instead of after 5–43 days); `regrowth.zeroThreshold` 0.4 (1% of the grass cap, the same fraction as before); `interventions.meadow.plants` 20 (half the grass cap, as before). With those values the probes gave, by day 48: seed 1 pop 276 (herb 267, carn 9), born 534 vs immig 12, plants 67%, edge share 24%; seed 8 pop 1,178 (herb 1,166, carn 12), born 2,049 vs immig 18, plants 29%, edge 37% — sustained, but overshooting and hunter-poor. A variant with `biteSize` 0.5 and `metabolism.base` 0.01 peaked at 395 and crashed to 73 with plants at 16%, so do not push bite and metabolism up together.
- Targets for this task's after-sweep (`--seeds 1..40 --ticks 100000`): `survived ≥ 30/40` (pop > 0, herb > 0, carn > 0); mean `bornPerImmig ≥ 10`; mean `plantsAvg% ≤ 90`; mean `immig ≤ 30`; mean `edge% ≤ 60`; mean `pop ≥ 100`; `refused` (capacity refusals) = 0 on ≥ 30/40 seeds — if the population hits `world.maxOrganisms` (2000) on many seeds, raise `breeding.localK` down or `breeding.radius` up rather than raising `maxOrganisms` (SPEC §4.1 sizes the world for 300–600 organisms on a phone). Boom-bust (a low `minPopY2`) is acceptable in this task and is P6-04's job; a crash to zero is not.
- Keys you may change in this task: `terrain.plantCap`, `plants.growth`, `plants.soilBoost`, `organisms.biteSize`, `metabolism.base`, `metabolism.moveCost`, `phenotype.lifespan`, `phenotype.maturity`, `phenotype.breedThreshold`, `breeding.baseRate`, `breeding.localK`, `breeding.radius`, `regrowth.zeroThreshold`, `regrowth.debtTicks`, `regrowth.debtFactor`, `interventions.meadow.plants`, `genesis.energyFraction`. Do not touch predation, immigration, disease, or genesis counts (P6-04, P6-05). Every changed default gets a one-line comment naming P6-03 and the reason, as P1-11's do. If the old pinned seeds (8 and 39 in `ecology.test.js`, 29 in `survival.test.js`) fail any existing assertion under the new defaults, re-pin from the after-sweep and record why; do not weaken an existing assertion.
- The invariant tests in `test/invariants/` must stay green untouched; in particular the throughput gate uses a fixed 64×40 / 200-organism scenario and is unaffected by defaults, but `test/invariants/energy.test.js` must close at the new scale.
**Acceptance tests:** `test/soak/health.test.js` (six tests above) green on both pinned seeds; `test/soak/ecology.test.js` and `test/soak/survival.test.js` green.
**Out of scope:** Code changes outside `config.js` defaults and the named test/doc files. Predation, immigration and genesis-count keys.
**Verification:** `npm run typecheck && npm run lint && npm test && npm run test:soak`; sweep before and after with the tables in `docs/tuning.md`, `docs/sweeps/`, and the commit message.
**Depends on:** P6-02

### P6-04: Damping and hunters — boom-bust, predator viability, immigration as a backstop
**Goal:** Turn the P6-03 oscillation into a sustained population in the 300–900 range with a viable carnivore niche, and demote immigration to a rare rescue.
**Files touched:** `src/core/config.js` (⚠️ defaults and comments only), `test/soak/health.test.js` (extend), `docs/tuning.md`, `docs/sweeps/p6-04-before.txt`, `docs/sweeps/p6-04-after.txt`.
**Design constraints:**
- Tests first: extend `test/soak/health.test.js` with `"the population does not crash: the year-2+ minimum is at least 25% of the run's maximum"` (`minPopY2 ≥ 0.25 × maxPop`, both sampled every 600 ticks; `maxPop` over the whole run), `"hunters persist: at least 15 carnivores alive at the end"`, `"immigration fires at most 5 times"`, `"no capacity refusals"` (`counters.capacityRefused === 0`). Run against the P6-03 defaults first and record which fail.
- Levers, in the order to try them: (1) density-dependent breeding (`breeding.localK`, `breeding.radius`, `breeding.baseRate`, `phenotype.breedThreshold`) to damp the herbivore overshoot; (2) predator income — `predation.killChance`, `predation.reach`, `predation.maxPreySizeRatio`, `energy.etaCarn`, `organisms.bodyMassPerSize` (a kill's carcass is the prey's `energy + body`, so body mass is the hunter's meal size) and `carcass.decay`/`carcass.decayMud` (how long a carcass stays edible), so a hunter's expected intake per kill covers its metabolism between kills; (3) `immigration.floorHerbivores` (aim 8), `immigration.floorCarnivores` (aim 2), `immigration.cooldownTicks` (aim ≥ 9,000 = 5 days), `immigration.groupSize`; (4) `disease.*` only if a monoculture guard fails. Do not change plant-scale keys from P6-03 except `plants.growth` by at most ±30% if the plant fill target drifts.
- After-sweep targets: `survived ≥ 32/40`; mean `pop` in `[300, 900]`; mean `minPopY2 ≥ 0.25 × mean max`; mean `carn ≥ 15` and `carn ≥ 5` on ≥ 32/40 seeds; mean `immig ≤ 5`; `refused` = 0 on ≥ 36/40; all P6-03 targets still met; mean `H` not below the P6-03 after-table by more than 10%. Record any target you could not reach with the closest table and the lever you believe is missing (that is a finding for the reviewer, not a reason to loosen the test — if a health assertion cannot pass on any seed, mark the task `[!]` per `/implement`).
**Acceptance tests:** `test/soak/health.test.js` (the four new tests) green on both pinned seeds; `test/soak/ecology.test.js`, `test/soak/survival.test.js` green.
**Out of scope:** Code changes; genesis counts; terrain.
**Verification:** `npm run typecheck && npm run lint && npm test && npm run test:soak`; sweep before/after.
**Depends on:** P6-03

### P6-05: Genesis size, lineage diversity and the edge bias
**Goal:** Start the world with enough founders and lineages to keep species diversity through year 4, and make the terrain's wetter-edges bias a config key so the map's centre is not the driest, rockiest band by a hard-coded constant.
**Files touched:** `src/core/terrain.js`, `src/core/config.js` (`terrain.edgeWetness` new key with DOCS; `genesis.*` defaults), `test/unit/terrain.test.js` (extend), `test/unit/genesis.test.js` (extend), `test/soak/health.test.js` (extend), `docs/tuning.md`, `docs/sweeps/p6-05-before.txt`, `docs/sweeps/p6-05-after.txt`.
**Design constraints:**
- Determinism (SPEC §3.1): `terrain.js` currently adds `0.05 * sin((x / w) * PI)` to the fbm value (a literal, which breaks CLAUDE.md rule 7). Replace the literal with `cfg.terrain.edgeWetness` (⚠️, default 0.05 so every existing seed's terrain is unchanged — `npm run headless -- --ticks 5000` hash identical before and after the code change; record both). Then tune it in the same task (a smaller value flattens the centre-is-driest gradient; 0 removes it). Changing it re-rolls every seed's terrain, so the pinned soak seeds may need re-pinning — allowed, with reasons.
- Genesis: `genesis.herbivoreLineages` (aim 4–5), `genesis.herbivoresPerLineage` (aim 60–80), `genesis.carnivoreLineages` (aim 2), `genesis.carnivoresPerLineage` (aim 24–32), `genesis.lineageNoise`, `genesis.clusterRadius`, and `species.theta` if splits collapse. Founders are placed at independently random land tiles (`genesis.js` `findLineageCentre`); leave that code alone in this task, and note in the log if carnivore lineages still land out of reach of any herbivore cluster (a finding for the reviewer).
- Tests first: extend `test/soak/health.test.js` with `"at least 6 living species at the end"` and `"mean Shannon diversity over the run is at least 1.2"`. After-sweep targets: mean living `species ≥ 6`; mean `avgH ≥ 1.5`; mean `edge% ≤ 45`; every P6-03 and P6-04 target still met.
**Acceptance tests:** `test/unit/terrain.test.js` (extend): `"terrain.edgeWetness = 0 removes the east–west wetness gradient and 0.05 reproduces the previous terrain byte for byte"` (compare against a hash or a stored 64×40 fixture generated before the change; state which in the log). `test/unit/genesis.test.js` (extend): `"genesis places herbivoreLineages × herbivoresPerLineage + carnivoreLineages × carnivoresPerLineage founders"` (at the new defaults). `test/soak/health.test.js` (the two new tests) green on the pinned seeds.
**Out of scope:** Changing `findLineageCentre`; UI.
**Verification:** `npm run typecheck && npm run lint && npm test && npm run test:soak`; sweep before/after; `npm run headless -- --ticks 5000` twice with identical hashes.
**Depends on:** P6-04

### P6-06: Phase 6 end — save version, performance re-check, docs, push, preview, phone checks
**Goal:** Close the rebalance: make old saves fail loudly instead of replaying into a different world, re-measure performance at the new population, refresh every number the docs cite, push, and list what a human must check on a phone.
**Files touched:** `src/core/save.js` (`VERSION` 1 → 2), `src/persist/autosave.js` and/or `src/persist/db.js` (only if a version mismatch on resume is not already handled by discarding the record and starting fresh — it must be, without a console error storm), `test/unit/save.test.js` (extend), `docs/performance.md`, `docs/tuning.md` (Phase 6 summary), `docs/development.md` ("Ecology rebalance (Phase 6)" section), `docs/blog-post.md` and `README.md` (only the numbers that came from headless runs — re-run the cited commands and replace the numbers; no new prose beyond a sentence noting the rebalance), `docs/PROGRESS.md`.
**Design constraints:**
- `save.restoreState` already throws on a version mismatch; the resume path must catch it, discard the autosave, and start a new world from the same seed with a single `console.warn`. Share links are `{ seed, configDiff, interventions[] }` and carry no version: a pre-Phase-6 link now replays under the new defaults (a different world). Record this in `docs/development.md` as the known consequence of retuning; do not add a config-snapshot to share links in this task.
- Performance: `node scripts/perf.mjs` at the new defaults (populations are now hundreds, not tens); write the numbers into `docs/performance.md` next to the P5-06 ones. If the default-world Node throughput has fallen below 50% of the P5-06 figure, record it as a finding for the reviewer — do not optimise in this task. `test/invariants/throughput.test.js` (fixed 200-organism gate) must still pass.
- Phase-end template (Conventions). Phone checklist verbatim: "(1) leave the sim at 16× for ten minutes: the population figure stays in the hundreds and the chronicle shows births and splits, not a stream of migrations; (2) grazed ground is visibly darker than ungrazed ground and herds move on when a patch is eaten down; (3) the inspector shows organisms older than 14 days without an old-age death wave; (4) herds are seen in the middle of the map, not only along the edges; (5) frame time at 16× still feels smooth with several hundred organisms on screen".
**Acceptance tests:** `test/unit/save.test.js` (extend): `"a version-1 record is refused by restoreState"` and `"resume discards a refused record and starts fresh from the seed"` (in whichever unit test already covers resume — `autosave.test.js` or `db.test.js` — name it in the log). `npm run test:all` and `npm run test:ui` green.
**Out of scope:** New mechanics; optimisation; UI changes.
**Verification:** `npm run typecheck && npm run lint && npm run test:all && npm run build && npm run test:ui`; `node scripts/perf.mjs`; `npm run headless -- --seed <each pinned seed> --ticks 100000` summaries into the log; `git push`.
**Depends on:** P6-05
...
- **§4.4 / §4.6 — units of the plant stock and of lifespan are not stated.** `p ∈ [0, cap(terrain)]` gives no unit for `cap`, and the Phase 1 build read it as ≤ 1 while organisms hold ~150 energy, which forced `plants.growth` to a regrowth-in-seconds value; lifespan was read as days while §4.3 defines a 24-day year and the design intent is multi-year lives. Resolution (Phase 6): `cap` is in the same energy units as organism energy, tens per tile; lifespan is in days with a range of years. Proposed SPEC edit: state both units in §4.4 and §4.6, and add to §9.3 the health invariants from P6-03/P6-04 (births ≥ 10× immigrations, mean plant fill 15–90%, year-2 minimum ≥ 25% of the maximum, ≥ 15 carnivores at the end).
